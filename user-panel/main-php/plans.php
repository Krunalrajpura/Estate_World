      ...
      <div class="col-md-4 ml-auto pricing-box align-self-center">
        <div class="card mb-4 myShadow myScale border-0 rounded-lg"
          ...
          <div class="card-body p-4 text-center">
            ...
            <p class="mt-3"><i class="fa-solid fa-x"></i> Get the Extra 10 Points</p>
            <a class="btn btn-sm rounded-pill text-white my-4" href="#" data-toggle="modal" data-target="#modal499"
              style="background-color: #10bb87;">CHOOSE PLAN
            </a>
          </div>
        </div>
      </div>
      ...
      <div class="col-md-4 ml-auto pricing-box align-self-center">
        <div class="card mb-4 myShadow myScale border-0 rounded-lg">
          <div class="card-body p-4 text-center">
            ...
            <a class="btn btn-sm rounded-pill text-white my-4" href="#" data-toggle="modal" data-target="#modal699"
              style="background-color: #5d78ff;">CHOOSE PLAN
            </a>
...
if (isset($_POST['pay'])) {
    ...
    $point          = $_POST['points']; // Credit points purchased

    // Fetch the c_id from the login table
    ...
    // Check if a record exists in tbl_plans_details
    $conditions = ['c_id' => $c_id];
    $result = checkIfDataExists($conn, 'tbl_plans_details', $conditions);

    if ($result) {
        // Update plan: add new points to existing points.
        ...
        // Insert new record into tbl_plans_details
        $dataInsert = [
            'c_id'     => $c_id,
            'o_name'   => $cardHolderName,
            'card_num' => $cardNumber,
            'exp_date' => date('Y-m-d', strtotime($expiryDate)),
            'cvv'      => $cvv,
            'amount'   => $amount,
            'points'   => $point,
        ];
